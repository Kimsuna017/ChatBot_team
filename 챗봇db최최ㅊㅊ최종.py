...
def test(bot, update):
    ...
    t = "타로와 심리 두가지 테마 중 원하는 걸 선택하세요.\n혹시 챗봇에 대하여 궁금한 사항이 있다면 Q&A를 눌러보세요!"
    ...
    show_list.append(InlineKeyboardButton("Q&A", callback_data="Q&A"))

    show_markup = InlineKeyboardMarkup(build_menu(show_list, len(show_list) - 1))

    update.message.reply_text("셋 중 고르세요", reply_markup=show_markup)
...
def callback_get(bot, update):
    ...
    if update.callback_query.data == "Q&A":

        bot.edit_message_text(text="/question 가 선택되었습니다".format(update.callback_query.data),
    ...
    if update.callback_query.data == "사용자":

        bot.edit_message_text(text="/user 가 선택되었습니다".format(update.callback_query.data),
    ...
    if update.callback_query.data == "인기":

        bot.edit_message_text(text="/recommand 가 선택되었습니다".format(update.callback_query.data),
    ...
    if update.callback_query.data == "세부사항":

        bot.edit_message_text(text="/detail 가 선택되었습니다".format(update.callback_query.data),
...
def question(bot, update):
    ...
    bot.sendMessage(chat_id=update.message.chat_id, text='궁금하신 메뉴를 선택해 주세요!')

    show_list = []

    show_list.append(InlineKeyboardButton("1. 총 이용자수", callback_data="사용자"))

    show_list.append(InlineKeyboardButton("2. 인기 메뉴", callback_data="인기"))

    show_list.append(InlineKeyboardButton("3.세부사항", callback_data="세부사항"))

    show_list.append(InlineKeyboardButton("4.취소", callback_data="취소"))

    show_markup = InlineKeyboardMarkup(build_menu(show_list, len(show_list) - 1))

    update.message.reply_text("궁금한 것을 골라줘!", reply_markup=show_markup)
...
def user(bot, update):
    ...
    bot.sendMessage(chat_id=update.message.chat_id, text='저희 챗봇의 총 이용자 수가 궁금하시다구요?')
    time.sleep(0.5)
    bot.sendMessage(chat_id=update.message.chat_id, text='저희 챗봇의 총 이용자 수는 \"' + str(Sum) + '\"명이에요!')
    time.sleep(0.5)
    bot.sendMessage(chat_id=update.message.chat_id, text='더 많은 이용 부탁 드릴게요!!')
    time.sleep(0.5)
    bot.sendMessage(chat_id=update.message.chat_id, text='또다른 궁금한 사항이 있다면 /question 을 눌러주세요~')
    time.sleep(0.5)
...
def recommand(bot, update):
    ...
    bot.sendMessage(chat_id=update.message.chat_id, text='무슨 메뉴를 골라야 할지 고민이면')
    time.sleep(0.5)
    bot.sendMessage(chat_id=update.message.chat_id, text='가장 인기있는 메뉴를 해보는건 어떨까요?')
    time.sleep(0.5)
    bot.sendMessage(chat_id=update.message.chat_id, text='저희 타로심리봇 이용 횟수는 총 \"' + str(Sum) +'\"명인데요!')
    time.sleep(0.5)
    bot.sendMessage(chat_id=update.message.chat_id, text='그 중')
    time.sleep(0.5)


    if (birth >= tod and birth >= door and birth >= animal and birth >= photo and birth >= number):
        Max = birth
        bot.sendMessage(chat_id=update.message.chat_id, text='타로테마의 생일점 이용자 수가 \"' + str(Max) + '\"명으로 가장 인기가 많네요!')
        time.sleep(0.5)
        bot.sendMessage(chat_id=update.message.chat_id, text='이 메뉴를 바로 이용해 보고 싶으시다면 /taro_b 를 눌러주세요!')
        time.sleep(0.5)
        count+=1
    if (tod >= birth and tod >= door and tod >= animal and tod >= photo and tod >= number):
        Max = tod
        if(count!=0):
            bot.sendMessage(chat_id=update.message.chat_id, text='그리고 공동으로')
            time.sleep(0.5)
        bot.sendMessage(chat_id=update.message.chat_id, text='타로테마의 오늘의 운세 이용자 수가 \"' + str(Max) + '\"명으로 가장 인기가 많네요!')
        time.sleep(0.5)
        bot.sendMessage(chat_id=update.message.chat_id, text='이 메뉴를 바로 이용해 보고 싶으시다면 /taro_d 를 눌러주세요!')
        time.sleep(0.5)
        count += 1
    if (door >= birth and door >= tod and door >= animal and door >= photo and door >= number):
        ...
        if (count != 0):
            bot.sendMessage(chat_id=update.message.chat_id, text='그리고 공동으로')
            time.sleep(0.5)
        bot.sendMessage(chat_id=update.message.chat_id, text='심리테마의 문 메뉴 이용자 수가 \"' + str(Max) + '\"명으로 가장 인기가 많네요!')
        time.sleep(0.5)
        bot.sendMessage(chat_id=update.message.chat_id, text='이 메뉴를 바로 이용해 보고 싶으시다면 /door 를 눌러주세요!')
        time.sleep(0.5)
        count += 1
    if (animal >= birth and animal >= tod and animal >= door and animal >= photo and animal >= number):
        ...
        if (count != 0):
            bot.sendMessage(chat_id=update.message.chat_id, text='그리고 공동으로')
            time.sleep(0.5)
        bot.sendMessage(chat_id=update.message.chat_id, text='심리테마의 동물 메뉴 이용자 수가 \"' + str(Max) + '\"명으로 가장 인기가 많네요!')
        time.sleep(0.5)
        bot.sendMessage(chat_id=update.message.chat_id, text='이 메뉴를 바로 이용해 보고 싶으시다면 /animal 를 눌러주세요!')
        time.sleep(0.5)
        count += 1
    if (photo >= birth and photo >= tod and photo >= door and photo >= animal and photo >= number):
        ...
        if (count != 0):
            bot.sendMessage(chat_id=update.message.chat_id, text='그리고 공동으로')
            time.sleep(0.5)
        bot.sendMessage(chat_id=update.message.chat_id, text='심리테마의 사진 메뉴 이용자 수가 \"' + str(Max) + '\"명으로 가장 인기가 많네요!')
        time.sleep(0.5)
        bot.sendMessage(chat_id=update.message.chat_id, text='이 메뉴를 바로 이용해 보고 싶으시다면 /photo 를 눌러주세요!')
        time.sleep(0.5)
        count += 1
    if (number >= birth and number >= tod and number >= door and number >= animal and number >= photo):
        ...
        if (count != 0):
            bot.sendMessage(chat_id=update.message.chat_id, text='그리고 공동으로')
            time.sleep(0.5)
        bot.sendMessage(chat_id=update.message.chat_id, text='심리테마의 숫자 메뉴 이용자 수가 \"' + str(Max) + '\"명으로 가장 인기가 많네요!')
        time.sleep(0.5)
        bot.sendMessage(chat_id=update.message.chat_id, text='이 메뉴를 바로 이용해 보고 싶으시다면 /number 를 눌러주세요!')
        time.sleep(0.5)
        count += 1
...
def detail(bot, update):
    ...
    bot.sendMessage(chat_id=update.message.chat_id, text='각 테마별 이용자 수를 알고 싶으시다구요?')
    time.sleep(0.5)
    bot.sendMessage(chat_id=update.message.chat_id, text='걱정하지 마세요! 제가 알려 드릴게요!!')
    time.sleep(0.5)
    bot.sendMessage(chat_id=update.message.chat_id, text='저희 타로심리봇 이용 횟수는 총 \"' + str(Sum) + '\"명인데요!')
    time.sleep(0.5)
    bot.sendMessage(chat_id=update.message.chat_id, text='그 중')
    time.sleep(0.5)
    bot.sendMessage(chat_id=update.message.chat_id, text='생일점 이용자 수는 \"' + str(birth) + '\"명')
    time.sleep(0.5)
    bot.sendMessage(chat_id=update.message.chat_id, text='오늘의 운세 이용자 수는 \"' + str(tod) + '\"명')
    time.sleep(0.5)
    bot.sendMessage(chat_id=update.message.chat_id, text='문 메뉴 이용자 수는 \"' + str(door) + '\"명')
    time.sleep(0.5)
    bot.sendMessage(chat_id=update.message.chat_id, text='동물 메뉴 이용자 수는 \"' + str(animal) + '\"명')
    time.sleep(0.5)
    bot.sendMessage(chat_id=update.message.chat_id, text='사진 메뉴 이용자 수는 \"' + str(photo) + '\"명')
    time.sleep(0.5)
    bot.sendMessage(chat_id=update.message.chat_id, text='숫자 메뉴 이용자 수는 \"' + str(number) + '\"명')
    time.sleep(0.5)
    bot.sendMessage(chat_id=update.message.chat_id, text='입니다! 더 다양한 메뉴들 많이 이용해주세요~')
    time.sleep(1.0)
    bot.sendMessage(chat_id=update.message.chat_id, text='또다른 궁금한 사항이 있다면 /question 을\n어떤 메뉴를 고릴지 결정하셨다면 /test 를 눌러주세요~')
...
def crawl_animal(bot, update):
    ...
    bot.sendMessage(chat_id=update.message.chat_id, text='이 중 가장 먼저 보이는 동물 혹은 곤충은 무엇인가요?')
...
def number_check(bot, update):
    ...
    bot.sendMessage(chat_id= update.message.chat_id, text= '제가 당신이 좋아하는 숫자를 맞춰보려고 하는데요!')
    time.sleep(1.0)
    bot.sendMessage(chat_id=update.message.chat_id, text='제가 지시하는 대로 따라해 주실래요?')
    time.sleep(1.0)
    bot.sendMessage(chat_id=update.message.chat_id, text='1부터 10 중에 아.무 숫자나 하나 골라 보세요!')
    time.sleep(1.0)
    bot.sendMessage(chat_id=update.message.chat_id, text='고른 숫자는 저를 알려주지 말고 마음속으로만 생각하세요!')
    time.sleep(1.0)
    bot.sendMessage(chat_id=update.message.chat_id, text='골랐나요? 골랐다면 /yes 를 눌러 주세요!')
...
def yes2(bot, update):
    bot.sendMessage(chat_id=update.message.chat_id, text='그 곱한 수가 한자리면 그냥 냅두고\n두자리라면 각 자리의 숫자를 더해주세요!\nex)4->4, 14->1+4=5')
    time.sleep(1.0)
...
if __name__ == '__main__':
    ...
    dp.add_handler(CommandHandler('recommand', recommand))
