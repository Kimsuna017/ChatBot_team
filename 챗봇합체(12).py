...
TEST, REALTARO, MENU, TARO_B, TARO_D, DOOR, ANIMAL, PICTURE, NUMBER, GREEN, BLUE, RED, PURPLE, GREY, QA, USER, RECOMMAND, DETAIL, A, B, C, D, CANCEL = range(
    23)
...
def door(bot, update):
    ...
    return CallbackQueryHandler(callback_get3)
...
def get_message(bot, update):
    ...
    if '타로점' in text and '타로' in text:
        ...
        bot.send_message(chat_id=update.message.chat_id,
                         text='아무리 험난하고 고된 시련이 와도 아름다움을 유지하는 당신은\n시련을 이겨낼 줄 아는 강인한 성격을 가진 타입입니다.')
        ...
        bot.send_message(chat_id=update.message.chat_id,
                         text='영리하고 현명한 당신은 주위 사람들에게 늘 침착하고 부드러운 성품을 보이며\n용기와 희망을 전해주고 있습니다.')
    ...
    if (int(update.message.text) > 22 and int(update.message.text) < 99):
        ...
        time.sleep(1.5)
        bot.sendMessage(chat_id=update.message.chat_id, text='또 다른 테스트를 하고 싶다면 /test 를 눌러주세요!')
...
def test(bot, update):
    ...
    return CallbackQueryHandler(callback_get1)
...
def realtaro(bot, update):
    ...
    return CallbackQueryHandler(callback_get2)
...
con, cur = None, None
global birth, tod, door1, animal, photo, number, Max, Min, Sum


# 버튼 처리할 함수
def callback_get1(bot, update):
    query = update.callback_query
...
def callback_get2(bot, update):
    ...
    if update.callback_query.data == str(TARO_B):
        ...
        callback_get3(bot, update)


def callback_get3(bot, update):
    query = update.callback_query
    ...
    if update.callback_query.data == str(GREEN):
        uu = green(bot, update)
...
if __name__ == '__main__':
    ...
    conv_handler2 = ConversationHandler(
        entry_points=[CallbackQueryHandler(callback_get1)],
        states={
        ...
        fallbacks=[CallbackQueryHandler(callback_get1)]
    )

    conv_handler3 = ConversationHandler(
        entry_points=[CallbackQueryHandler(callback_get2)],
        states={
        ...
        fallbacks=[CallbackQueryHandler(callback_get2)]
    )

    conv_handler4 = ConversationHandler(
        entry_points=[CallbackQueryHandler(callback_get3)],
        states={
        ...
        fallbacks=[CallbackQueryHandler(callback_get3)]
    )
