...
def today(bot, update):
    ...
    if value == 0 :
        ...
        update.message.reply_text("또다른 테스트를 하고 싶다면 /start 를 눌러보세요!")
...
def green(bot, update):
    ...
    bot.send_message(
        chat_id=query.message.chat_id,
        message_id=query.message.message_id,
        text=u"다른 테스트를 하고 싶다면 /start 를 누르세요!"
    )
...
def get_message(bot, update):
    ...
    if '타로점' in text and '타로' in text:
        ...
        bot.send_message(chat_id=
                         update.message.chat_id, text='닭이 먼저 보였다면 인내심이 강한 타입입니다!')
    ...
    if (int(update.message.text) > 22 and int(update.message.text) < 99):
        ...
        yes24.insert(0, t)
        bot.sendMessage(chat_id=update.message.chat_id, text='당신이 좋아하는 숫자는 바로바로!')
        time.sleep(1.0)
        bot.sendMessage(chat_id=update.message.chat_id, text='두둥...!!')
        time.sleep(1.0)
        uu = yes5()
        bot.sendMessage(chat_id=update.message.chat_id, text=str(uu))
        time.sleep(1.0)
        bot.sendMessage(chat_id=update.message.chat_id, text='정답이죠?ㅎㅎ')
        time.sleep(1.5)
        bot.sendMessage(chat_id=update.message.chat_id, text='또 다른 테스트를 하고 싶다면 /start 를 눌러주세요!')
...
def start(bot, update):
    ...
    update.message.reply_text(
        u"안녕하세요! 저는 Mr.Sim 이에요! 시작하기를 원하면 test 를 눌러주세요~",
...
def test(bot, update):
    ...
    bot.edit_message_text(
        chat_id=query.message.chat_id,
        message_id=query.message.message_id,
        text=u"원하는 걸 선택하세요!"+"\n"+"어떤 것을 선택해야 할지 모르겠다면 Q&A 버튼을 눌러보세요~",
    )
...
def taro_b(bot, update):
    ...
    bot.send_message(
        chat_id=query.message.chat_id,
        message_id=query.message.message_id,
        text=u"당신의 생일을 이용한 성격카드를 보여주려고 하는데요!"
    )
...
def callback_get(bot, update):
    ...
    if update.callback_query.data == str(REALTARO):
        ...
        sql = "INSERT INTO chatbot(sel1, sel2, sel3) VALUES('타로','성격카드','')"
        cur.execute(sql)
...
#DB 저장 및 값가져오기
    ...
    Sum = data[0]
    print(Sum)
    cur.execute("select count(*) from chatbot where sel2='성격카드'")
    data = cur.fetchone()
    birth = data[0]
    print(birth)
...
def user(bot, update):
    ...
    bot.send_message(
        chat_id=query.message.chat_id,
        message_id=query.message.message_id,
        text=u"또다른 궁금한 사항이 있다면 /start 를 누른후 Q&A를 확인해주세요~"
    )
    time.sleep(0.5)
...
def recommand(bot, update):
    ...
    if (birth >= tod and birth >= door and birth >= animal and birth >= photo and birth >= number):
        ...
        bot.send_message(
            chat_id=query.message.chat_id,
            message_id=query.message.message_id,
            text=u'타로테마의 성격카드 이용자 수가 \"' + str(Max) + '\"명으로 가장 인기가 많네요!'
        )
        time.sleep(0.5)
        bot.send_message(
            chat_id=query.message.chat_id,
            message_id=query.message.message_id,
            text=u"이 메뉴를 바로 이용해 보고 싶으시다면 /start 를 누른 후 '타로->성격카드'을 차례로 클릭해보세요!"
        )
        time.sleep(0.5)
        count+=1
    if (tod >= birth and tod >= door and tod >= animal and tod >= photo and tod >= number):
        ...
        bot.send_message(
            chat_id=query.message.chat_id,
            message_id=query.message.message_id,
            text=u"이 메뉴를 바로 이용해 보고 싶으시다면 /start 를 누른 후 '타로->오늘의 운세'를 차례로 클릭해보세요!"
        )
        time.sleep(0.5)
        count += 1
    if (door >= birth and door >= tod and door >= animal and door >= photo and door >= number):
        ...
        bot.send_message(
            chat_id=query.message.chat_id,
            message_id=query.message.message_id,
            text=u"이 메뉴를 바로 이용해 보고 싶으시다면 /start 를 누른 후 '심리->문'을 차례로 클릭해보세요!"
        )
        time.sleep(0.5)
        count += 1
    if (animal >= birth and animal >= tod and animal >= door and animal >= photo and animal >= number):
        ...
        bot.send_message(
            chat_id=query.message.chat_id,
            message_id=query.message.message_id,
            text=u"이 메뉴를 바로 이용해 보고 싶으시다면 /start 를 누른 후 '심리->동물'을 차례로 클릭해보세요!"
        )
        time.sleep(0.5)
        count += 1
    if (photo >= birth and photo >= tod and photo >= door and photo >= animal and photo >= number):
        ...
        bot.send_message(
            chat_id=query.message.chat_id,
            message_id=query.message.message_id,
            text=u"이 메뉴를 바로 이용해 보고 싶으시다면 /start 를 누른 후 '심리->사진'을 차례로 클릭해보세요!"
        )
        time.sleep(0.5)
        count += 1
    if (number >= birth and number >= tod and number >= door and number >= animal and number >= photo):
        ...
        bot.send_message(
            chat_id=query.message.chat_id,
            message_id=query.message.message_id,
            text=u"이 메뉴를 바로 이용해 보고 싶으시다면 /start 를 누른 후 '심리->숫자'를 차례로 클릭해보세요!"
        )
        time.sleep(0.5)
        count += 1
...
def detail(bot, update):
    ...
    bot.send_message(
        chat_id=query.message.chat_id,
        message_id=query.message.message_id,
        text=u'성격카드 이용자 수는 \"' + str(birth) + '\"명'
    )
    time.sleep(0.5)
    bot.send_message(
        chat_id=query.message.chat_id,
...
if __name__ == '__main__':
    bot.sendMessage(chat_id=chat_id, text='안녕하세요! 저는 타로, 심리 봇 Mr.Sim이에요! 시작하기를 원하면 /start 를 눌러주세요~')
